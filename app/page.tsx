  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white">
        <div className="max-w-7xl mx-auto px-4 py-16 sm:px-6 lg:px-20">
          <div className="text-center">
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold">{data?.stats.totalCountries || 0}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-md p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm text-gray-500">Top Industry</p>
                <p className="text-2xl font-bold truncate">{data?.stats.topIndustry || 'N/A'}</p>
                <p className="text-xs text-gray-500">{data?.stats.topIndustryCount.toLocaleString()} workers</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {data?.topCompanies.slice(0, 5).map((company, i) => (
                <Link key={i} href={`/companies/${encodeURIComponent(company.name)}`} className="flex justify-between items-center px-6 py-3 hover:bg-gray-50 transition">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className="text-lg font-semibold text-blue-600">{company.currentYearPermits}</span>
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
              <h2 className="text-lg font-semibold">Top Industries by Sponsorships</h2>
            </div>
            <div className="divide-y divide-gray-200">
              {data?.topIndustries.map((industry, i) => (
                <Link key={i} href={`/companies?industry=${encodeURIComponent(industry.name)}`} className="flex justify-between items-center px-6 py-3 hover:bg-gray-50 transition">
                  <p className="font-medium text-gray-900">{industry.name}</p>
                  <p className="text-lg font-semibold text-green-600">{industry.count.toLocaleString()}</p>
                </Link>
              ))}
            </div>
          </div>
